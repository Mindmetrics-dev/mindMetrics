...
# Registro diario
# ─────────────────────────────────────────────────────────────────────────────
from .forms import EMOCION_CHOICES, RegistroDiarioForm  # noqa: E402
...
def registro_diario(request: HttpRequest) -> HttpResponse:
    """
    GET  — Muestra el formulario del registro diario.
    POST — Valida, delega al service y redirige al dashboard.

    El service (formulario/services.py) hace toda la orquestación:
    crea el RegistroEmocional, ejecuta el motor con el estado consolidado,
    y guarda el nivel_riesgo en el mismo registro. Todo transaccional.

    Reglas del flujo:
      - El usuario debe tener evaluacion_completada=True (lo garantiza el
        middleware EvaluacionInicialRequiredMiddleware).
      - Un solo registro por usuario por día (validado en el service).
    """
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                messages.warning(
                    request,
                    "Ya registraste tu diario hoy. Vuelve mañana para registrar otro.",
                )
                return redirect("dashboard")
            except services.EvaluacionInicialFaltante:
                messages.error(
                    request,
                    "Necesitas completar tu evaluación inicial antes de registrar el diario.",
                )
                return redirect("evaluacion_inicial")
            except Exception:
                logger.exception(
                    "Error en el flujo de registro diario para usuario %s",
                    request.user.pk,
                )
                messages.error(
                    request,
                    "Ocurrió un error al guardar tu registro. Inténtalo de nuevo.",
                )
                return redirect("registro_diario")

            # Guardar resultado en sesión para mostrarlo en la página de resultado
            nivel = registro.nivel_riesgo or ""
            request.session["resultado_diario"] = {
                "nivel": nivel,
                "etiqueta": _NIVEL_LEGIBLE.get(nivel, nivel.replace("_", " ").title() if nivel else "Sin evaluar"),
                "clase": _NIVEL_CLASE.get(nivel, "neutral"),
                "alerta": nivel in ("alto", "critico"),
            }
            return redirect("resultado_diario")
    else:
...
def resultado_diario(request: HttpRequest) -> HttpResponse:
    """
    Muestra el nivel de riesgo calculado tras guardar el registro diario.
    Lee los datos desde la sesión (puestos por registro_diario).
    Si no hay datos en sesión, redirige al dashboard.
    """
